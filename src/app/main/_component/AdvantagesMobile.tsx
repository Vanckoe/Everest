...
export default function AdvantagesMobile() {
  return (
<section className="flex flex-col px-10 py-8 gap-8 w-full">
  {/* Заголовок сверху */}
  ...
  {/* Основной заголовок */}
  <h2 className="text-[#001f3f] text-[32px] font-bold leading-tight text-left w-full max-w-3xl">
    Why 92% of clients recommend us to their friends?
  </h2>

  {/* Контейнер преимуществ */}
  <div className="w-full mx-auto grid grid-rows-4 grid-cols-1">
    ...
    {advantages.map(({ Icon, title, subtitle }, idx) => (
      <div
        key={idx}
        className={`
          flex flex-col gap-4 px-[30px] py-[25px] items-start
          box-border
          w-full
          border-l border-r border-[#1212120F]
          ${idx === 0 ? 'border-t' : ''}
          ${idx === advantages.length - 1 ? 'border-b' : 'border-b'}
        `}
      >
        <Icon className="w-[40px] h-[40px] text-[#001f3f]" />
        <h3 className="text-4xl font-semibold text-[#121212] leading-snug">{title}</h3>
        <p className="text-[#888888] text-2xl leading-snug">{subtitle}</p>
      </div>
    ))}
  </div>
</section>
