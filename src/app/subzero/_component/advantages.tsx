      ...
      <div className="hidden md:flex px-10 py-[4.5rem]">
        ...
        <div className="w-full flex flex-col">
          {advantages.map(({ title, subtitle }, idx) => (
            ...
            >
              {/* Номер */}
              <span className="text-[#1C2C4C] font-inter-semibold text-base w-[2rem] shrink-0 mt-1">
              ...
              <div className="flex items-start justify-between gap-30">
                {/* Заголовок */}
                <h3 className="text-color text-2xl font-inter-semibold leading-snug max-w-[22rem]">
                  {title}
                </h3>

                {/* Подзаголовок */}
                <p className="text-[#7D7D7D] font-inter text-base max-w-[24rem] leading-relaxed text-left">
                  {subtitle}
                </p>
              </div>
            </div>
